...
def InsertProduct(request):
    if request.method == 'POST':
        if request.POST['name'] and request.POST['price'] and request.POST['description'] and request.POST['color'] and \
                request.POST['category'] and request.FILES['image'] and request.POST['quantity']:
            product = Item()
...
def SortedProductList(request, keyword):
    if keyword == 'allclothes':
        item = Item.objects.filter(category__icontains='menclothes') or Item.objects.filter(
            category__icontains='menclothes womenclothes')
        return render(request, 'product/product_list.html', {'allproducts': item})
    else:
